...
class QadPEDITCommandClass(QadCommandClass):
    ...
    def connectQAction(self, action):
        action.triggered = self.plugIn.runPEDITCommand
    ...
    def __init__(self, plugIn):
        ...
        self.atSubGeom = None
        self.linearObjectList = QadLinearObjectList()
        ...
        self.snapPointsDisplayManager.setIconSize(
            QadVariables.get(QadMsg.translate("Environment variables", "AUTOSNAPSIZE")))
        self.snapPointsDisplayManager.setColor(
        ...
        if self.entity.isInitialized():  # selezionato solo un oggetto
            ...
            for layerEntitySet in self.entitySet.layerEntitySetList:
                ...
                for featureId in layerEntitySet.featureIds:
                    ...
                    # trasformo la geometria nel crs del canvas per lavorare con coordinate piane xy
                    updGeom = qad_utils.closeQgsGeometry(self.mapToLayerCoordinates(layer, f.geometry()), toClose,
            ...
            for layerEntitySet in self.entitySet.layerEntitySetList:
                ...
                for featureId in layerEntitySet.featureIds:
                    ...
                    if updGeom is not None:
                        ...
                        # plugIn, layer, features, refresh, check_validity
                if qad_layer.updateFeaturesToLayer(self.plugIn, layerEntitySet.layer, updObjects, False,
                                                   False) == False:
                    self.plugIn.destroyEditCommand()
                    return
    ...
    def simplify(self):
        ...
        if self.entity.isInitialized():  # selezionato solo un oggetto
            ...
            updSubGeom = QgsGeometry.fromPolyline(self.linearObjectList.asPolyline(tolerance2ApproxCurve)).simplify(
                self.simplifyTolerance)
            updGeom = qad_utils.setSubGeom(geom, updSubGeom, self.atSubGeom)
            if updGeom is not None:
                # trasformo la geometria nel crs del layer
                f.setGeometry(self.mapToLayerCoordinates(layer, updGeom))
    ...
    def breakFromVertexAtToSecondVertexAt(self):
        ...
        result = qad_utils.breakQgsGeometry(
            QgsGeometry.fromPolyline(self.linearObjectList.asPolyline(tolerance2ApproxCurve)), \
            firstPt, secondPt, \
            tolerance2ApproxCurve)
        if result is None:
            return
    ...
    def waitForEntsel(self):
        ...
        prompt = QadMsg.translate("Command_PEDIT", "Select polyline or [{0}]: ").format(
            QadMsg.translate("Command_PEDIT", "Multiple"))
    ...
    def waitForDistance(self):
        ...
        prompt = QadMsg.translate("Command_PEDIT", "Specify gap tolerance or [{0}] <{1}>: ").format(keyWords, str(
    ...
    def WaitForVertexEditingMenu(self):
        ...
        prompt = QadMsg.translate("Command_PEDIT", "Enter a vertex editing option [{0}] <{1}>: ").format(keyWords,
                                                                                                         self.default)
    ...
    def WaitForSecondVertex(self):
        ...
        prompt = QadMsg.translate("Command_PEDIT",
                                  "Enter a selection option for the second vertex [{0}] <{1}>: ").format(keyWords,
                                                                                                         self.default1)
...
class QadGRIPINSERTREMOVEVERTEXCommandClass(QadCommandClass):
    ...
    def waitForNewVertex(self):
        ...
        prompt = QadMsg.translate("Command_GRIPINSERTREMOVEVERTEX",
                                  "Specify the position of the new vertex or [{0}]: ").format(keyWords)
...
class QadGRIPARCLINECONVERTCommandClass(QadCommandClass):
    ...
    def waitForConvertToArc(self):
        ...
        keyWords = QadMsg.translate("Command_GRIP", "Copy") + "/" + \
                   QadMsg.translate("Command_GRIP", "Undo") + "/" + \
                   QadMsg.translate("Command_GRIP", "eXit")
        prompt = QadMsg.translate("Command_GRIPARCLINECONVERT", "Specify the arc middle point or [{0}]: ").format(
            keyWords)
