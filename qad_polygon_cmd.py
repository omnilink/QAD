...
from qad.qad_polygon_maptool import *
...
class QadPOLYGONCommandClass(QadCommandClass):
    ...
    def connectQAction(self, action):
        action.triggered = self.plugIn.runPOLYGONCommand
    ...
    def run(self, msgMapTool=False, msg=None):
        ...
        # RISPOSTA ALLA RICHIESTA DI POLIGONO INSCRITTO O CIRCOSCRITTO (da step = 2)
            ...
            if type(value) == unicode:
                ...
                if self.constructionModeByCenter == QadMsg.translate("Command_POLYGON",
                                                                     "Area") or self.constructionModeByCenter == "Area":
                    self.WaitForArea()
                else:
        ...
        # RISPOSTA ALLA RICHIESTA DEL RAGGIO (da step = 3)
            ...
            if type(value) == QgsPoint or type(value) == float:  # se é stato inserito il raggio del cerchio
                ...
                self.vertices.extend(
                    qad_utils.getPolygonByNsidesCenterRadius(self.sideNumber, self.centerPt, self.radius, \
                                                             mode, ptStart))
