...
from qad.qad_offset_maptool import *
...
class QadOFFSETCommandClass(QadCommandClass):
    ...
    def waitForDistance(self):
        ...
        if self.offSet < 0:
            ...
            default = self.offSet
        prompt = QadMsg.translate("Command_OFFSET", "Specify the offset distance or [{0}] <{1}>: ").format(keyWords,
                                                                                                           str(default))
    ...
    def waitForObjectSel(self):
        ...
        prompt = QadMsg.translate("Command_OFFSET", "Select object to offset or [{0}] <{1}>: ").format(keyWords,
                                                                                                       default)
    ...
    def run(self, msgMapTool=False, msg=None):
        ...
        if self.step == 0:  # inizio del comando
            ...
            CurrSettingsMsg = CurrSettingsMsg + QadMsg.translate("Command_OFFSET", "OFFSETGAPTYPE = ") + str(
                self.gapType)
            if self.gapType == 0:
        ...
        # RISPOSTA ALLA RICHIESTA DELLA DISTANZA DI OFFSET (da step = 0)
            ...
            if type(value) == unicode:
                if value == QadMsg.translate("Command_OFFSET", "Through") or value == "Through":
                    ...
                    prompt = QadMsg.translate("Command_OFFSET",
                                              "Erase source object after offsetting ? [{0}] <{1}>: ").format(keyWords,
                                                                                                             default)
