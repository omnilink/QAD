...
from qad.qad_rotate_maptool import *
...
class QadROTATECommandClass(QadCommandClass):
    ...
    def rotate(self, f, basePt, angle, rotFldName, layerEntitySet, entitySet):
        ...
        if dimEntity is None:
            ...
            if len(rotFldName) > 0:
                ...
                rotValue = 0 if rotValue is None or isinstance(rotValue, QPyNullVariant) else qad_utils.toRadians(
                    rotValue)  # la rotazione é in gradi nel campo della feature
    ...
    def waitForNewReferenceRot(self):
        ...
        prompt = QadMsg.translate("Command_ROTATE", "Specify new angle or [{0}] <{1}>: ").format(keyWords, str(
            qad_utils.toDegrees(angle)))
        ...
        # RISPOSTA ALLA RICHIESTA PRIMO PUNTO PER ANGOLO ROTAZIONE DI RIFERIMENTO (da step = 3)
            ...
            if type(value) == float:  # se é stato inserito l'angolo di rotazione
                ...
                self.waitForNewReferenceRot()
                    ...
                    value) == QgsPoint:  # se é stato inserito l'angolo di rotazione con un punto
                self.Pt1ReferenceAng = QgsPoint(value)
                self.getPointMapTool().Pt1ReferenceAng = self.Pt1ReferenceAng
                # imposto il map tool
                self.getPointMapTool().setMode(
                    Qad_rotate_maptool_ModeEnum.FIRST_PT_KNOWN_ASK_FOR_SECOND_PT_REFERENCE_ANG)
                # si appresta ad attendere un punto
