...
from qad.qad_cacheareas import QadLayerCacheGeomsDict
...
class QadGetPoint(QgsMapTool):
    ...
    def __init__(self, plugIn, drawMode=QadGetPointDrawModeEnum.NONE):
        ...
        self.__QadSnapper.setSnapLayers(
            qad_utils.getVisibleVectorLayers(self.canvas))  # Tutti i layer vettoriali visibili
        self.__QadSnapper.setProgressDistance(
            QadVariables.get(QadMsg.translate("Environment variables", "OSPROGRDISTANCE")))
        ...
        ToleranceInMapUnits = QadVariables.get(
            QadMsg.translate("Environment variables", "PICKBOX")) * self.canvas.mapSettings().mapUnitsPerPixel()
        ...
        self.__QadSnapPointsDisplayManager.setIconSize(
            QadVariables.get(QadMsg.translate("Environment variables", "AUTOSNAPSIZE")))
        self.__QadSnapPointsDisplayManager.setColor(
            QColor(QadVariables.get(QadMsg.translate("Environment variables", "AUTOSNAPCOLOR"))))
    ...
    def removeItems(self):
        ...
        if self.__RubberBand is not None:
            self.canvas.scene().removeItem(
                self.__RubberBand)  # prima lo stacco dal canvas altrimenti non si rimuove perchè usato da canvas
    ...
    def setDrawMode(self, drawMode):
        ...
        if self.__drawMode == QadGetPointDrawModeEnum.ELASTIC_LINE:
            ...
            self.__RubberBand = createRubberBand(self.canvas, QGis.Polygon, False, None,
                                                 self.rectangleCrossingSelectionColor)
            self.__RubberBand.setLineStyle(Qt.DotLine)
    ...
    def setSelectionMode(self, selectionMode):
        ...
        if selectionMode == QadGetPointSelectionModeEnum.POINT_SELECTION:
            if QadVariables.get(QadMsg.translate("Environment variables", "APBOX")) == 0:
                ...
                self.setCursorType(
                    QadCursorTypeEnum.CROSS | QadCursorTypeEnum.APERTURE)  # una croce + un quadratino usati per selezionare un punto
    ...
    def appendTmpGeometry(self, geom, CRS=None):
        ...
        if CRS is not None and CRS != self.canvas.mapSettings().destinationCrs():
            ...
            coordTransform = QgsCoordinateTransform(CRS,
                                                    self.canvas.mapSettings().destinationCrs())  # trasformo la geometria
    ...
    def getRealPolarAngOffset(self):
        ...
        if (
                self.__PolarMode is not None and self.__PolarMode & QadPOLARMODEnum.MEASURE_RELATIVE_ANGLE):  # (relativo al coeff angolare dell'ultimo segmento)
            ...
            if self.__QadSnapper is not None:
                ...
                if useOSnapPointsForPolar:
                    self.__QadSnapper.toggleReferenceLines(geom, point, crs, oSnapPointsForPolar,
                                                           self.__snapMarkerSizeInMapUnits)
                else:
    ...
    def canvasMoveEventOnPointSel(self, event):
        ...
        if result is not None:
            ...
            if (self.__prevGeom is None) or not self.__prevGeom.equals(geometry):
                ...
                runToggleReferenceLines = lambda: self.toggleReferenceLines(self.__prevGeom, layer.crs(), allSnapPoints,
                                                                            self.tmpShiftKey)
            ...
            boxSize = QadVariables.get(
                QadMsg.translate("Environment variables", "APERTURE"))  # leggo la dimensione del quadrato (in pixel)
