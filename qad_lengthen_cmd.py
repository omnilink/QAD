...
from qad.qad_lengthen_maptool import *
...
class QadLENGTHENCommandClass(QadCommandClass):
    ...
    def lengthen(self, point):
        ...
        if self.OpMode == "DElta":
            ...
            if linearObject.isSegment():
                newPt = qad_utils.getPerpendicularPointOnInfinityLine(linearObject.getStartPt(),
                                                                      linearObject.getEndPt(), transformedPt)
            else:  # arco
                newPt = qad_utils.getPolarPointByPtAngle(linearObject.getArc().center, \
                                                         qad_utils.getAngleBy2Pts(linearObject.getArc().center,
                                                                                  transformedPt), \
    ...
    def showLength(self, entity, pt):
        ...
        LinearObjectListToMisure.transformFromCRSToCRS(entity.layer.crs(),
    ...
    def waitForObjectSelToMisure(self):
        ...
        if self.defaultValue is None:
            prompt = QadMsg.translate("Command_LENGTHEN", "Select an object or [{0}] <{1}>:  ").format(keyWords,
                                                                                                       self.defaultValue)
        else:
            prompt = QadMsg.translate("Command_LENGTHEN", "Select an object or [{0}] <{1}>:  ").format(keyWords,
                                                                                                       self.defaultValue)
    ...
    def waitForDelta(self):
        ...
        prompt = QadMsg.translate("Command_LENGTHEN", "Enter delta length or [{0}] <{1}>: ").format(keyWords, str(
    ...
    def waitForObjectSel(self):
        ...
        prompt = QadMsg.translate("Command_LENGTHEN", "Select an object to change or [{0}]: ").format(
            QadMsg.translate("Command_LENGTHEN", "Undo"))
    ...
    def waitForTotal(self):
        ...
        keyWords = QadMsg.translate("Command_LENGTHEN", "Angle")
        prompt = QadMsg.translate("Command_LENGTHEN", "Specify total length or [{0}] <{1}>: ").format(keyWords, str(
...
class QadGRIPLENGTHENCommandClass(QadCommandClass):
    ...
    def lengthen(self, point):
        ...
        if linearObject.isSegment():
            newPt = qad_utils.getPerpendicularPointOnInfinityLine(linearObject.getStartPt(), linearObject.getEndPt(),
                                                                  point)
        else:  # arco
            newPt = qad_utils.getPolarPointByPtAngle(linearObject.getArc().center, \
