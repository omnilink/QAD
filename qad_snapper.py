...
class QadSnapper():
    ...
    def getOsnapPtAndLinesForPolar(self, point, polarAng, polarAngOffset):
        ...
        for item in self.__oSnapPointsForPolar.items():
            ...
            if item[0] != QadSnapTypeEnum.POLAR:
                for startPoint in item[1]:
                    pts = self.getPolarCoord(startPoint, point, polarAng,
                                             polarAngOffset)  # ritorna una lista con un solo punto
                    if len(pts) > 0:
        ...
        if self.__startPoint is not None:
            pts = self.getPolarCoord(self.__startPoint, point, polarAng,
                                     polarAngOffset)  # ritorna una lista con un solo punto
            if len(pts) > 0:
                self.__appendUniquePoint(result, pts[0])  # senza duplicazione
    ...
    # getSnapPoint
    # ===========================================================================
    def getSnapPoint(self, geom, point, CRS, excludePoints=None, polarAng=None, polarAngOffset=None,
                     isTemporaryGeom=False):
        """
        ...
        for iLayer in self.__snapLayers:  # ciclo sui layer da controllare
            if (iLayer.type() == QgsMapLayer.VectorLayer):
                ...
                if CRS is None:  # se non c'é CRS la geom si intende nel sistema di coord dei punti di snap
                    coordTransform = QgsCoordinateTransform(self.getSnapPointCRS(),
                                                            iLayerCRS)  # trasformo in coord ilayer
                ...
                for feature in iLayer.getFeatures(
                        qad_utils.getFeatureRequest([], True, geom_iLayerCoords.boundingBox(), True)):
                    g2 = self.__transformGeomToSnapPointCRS(feature.geometry(),
                                                            iLayerCRS)  # trasformo la geometria in coord dei punti di snap
    ...
    def __appendUniquePoint(self, pointList, point, CRS=None):
        ...
        while lo < hi:
            ...
            if self.__comparePts(pointList[mid], point) == -1:
                lo = mid + 1
            else:
                hi = mid
...
def str2snapParams(s):
    ...
    for snapTypeStr in snapTypeStrList:
        ...
        if string.find(snapTypeStr, QadMsg.translate("Snap", "PR")) == 0 or \
            ...
            # la parte successiva PR può essere vuota o numerica
            if string.find(snapTypeStr, QadMsg.translate("Snap", "PR")) == 0:
                param = qad_utils.str2float(
                    snapTypeStr[len(QadMsg.translate("Snap", "PR")):])  # fino alla fine della stringa
            else:
