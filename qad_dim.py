...
class QadDimStyle():
    ...
    def save(self, path="", overwrite=True):
      ...
      """
        ...
        # layer e loro caratteristiche
        config.set("dimension_options", "textualLayerName",
                   "" if self.textualLayerName is None else self.textualLayerName)
    ...
    def getInValidErrMsg(self):
      ...
      """
        ...
        if qad_layer.isTextLayer(self.getTextualLayer()) == False:
            errPartial = QadMsg.translate("Dimension",
                                          "has the textual layer for dimension ({0}) which is not a textual layer.")
        ...
        if qad_layer.isSymbolLayer(self.getSymbolLayer()) == False:
            errPartial = QadMsg.translate("Dimension",
                                          "has the symbol layer for dimension ({0}) which is not a symbol layer.")
        ...
        if (self.getLinearLayer().type() != QgsMapLayer.VectorLayer) or (
                self.getLinearLayer().geometryType() != QGis.Line):
            errPartial = QadMsg.translate("Dimension",
                                          "has the linear layer for dimension ({0}) which is not a linear layer.")
            errMsg = prefix + errPartial.format(self.getSymbolLayer().name())
            return errMsg
        # i layer devono avere lo stesso sistema di coordinate
        if not (
                self.getTextualLayer().crs() == self.getLinearLayer().crs() and self.getLinearLayer().crs() == self.getSymbolLayer().crs()):
            errMsg = prefix + QadMsg.translate("Dimension",
                                               "has not the layers with the same coordinate reference system.")
            return errMsg
    ...
    def textCommitChangesOnSave(self, plugIn):
      ...
      """
        ...
        if self.getTextualLayer() is not None:
            ...
            plugIn.layerStatusList.setStatus(self.getTextualLayer().id(),
                                             qad_layer.QadLayerStatusEnum.COMMIT_BY_INTERNAL)
    ...
    def updateTextReferencesOnSave(self, plugIn, textAddedEntitySet):
      ...
      """
        ...
        plugIn.beginEditCommand("Dimension recoded",
                                [self.getSymbolLayer(), self.getLinearLayer(), self.getTextualLayer()])
    ...
    def commitChanges(self, plugIn):
        ...
        if (excludedLayer is None) or excludedLayer.id() != self.getLinearLayer().id():
            # segno che questo layer è salvato da QAD
            plugIn.layerStatusList.setStatus(self.getLinearLayer().id(),
                                             qad_layer.QadLayerStatusEnum.COMMIT_BY_INTERNAL)
        ...
        if (excludedLayer is None) or excludedLayer.id() != self.getSymbolLayer().id():
            # segno che questo layer è salvato da QAD
            plugIn.layerStatusList.setStatus(self.getSymbolLayer().id(),
                                             qad_layer.QadLayerStatusEnum.COMMIT_BY_INTERNAL)
    ...
    def addDimEntityToLayers(self, plugIn, dimEntity):
      ...
      """
        plugIn.beginEditCommand("Dimension added",
                                [self.getSymbolLayer(), self.getLinearLayer(), self.getTextualLayer()])

        # prima di tutto inserisco il testo di quota
        # plugIn, layer, feature, coordTransform, refresh, check_validity
        if qad_layer.addFeatureToLayer(plugIn, self.getTextualLayer(), dimEntity.textualFeature, None, False,
                                       False) == False:
        ...
        if self.setDimId(dimId, [dimEntity.textualFeature], False) == True:  # setto id
            ...
            if qad_layer.updateFeatureToLayer(plugIn, self.getTextualLayer(), dimEntity.textualFeature, False,
                                              False) == False:
                plugIn.destroyEditCommand()
                return False
        ...
        if qad_layer.addFeaturesToLayer(plugIn, self.getSymbolLayer(), dimEntity.symbolFeatures, None, False,
                                        False) == False:
            plugIn.destroyEditCommand()
            return False
        ...
        # plugIn, layer, features, coordTransform, refresh, check_validity
        if qad_layer.addFeaturesToLayer(plugIn, self.getLinearLayer(), dimEntity.linearFeatures, None, False,
                                        False) == False:
            plugIn.destroyEditCommand()
            return False
    ...
    def getBlocksRotOnArc(self, dimLineArc, inside):
      ...
      inside = flag di modo, se = true le frecce sono interne altrimenti sono esterne
      """
        rot1 = dimLineArc.getTanDirectionOnPt(
            dimLineArc.getStartPt())  # angolo della linea di quota all'inizio dell'arco
    ...
    def getSpaceForBlock1AndBlock2OnLine(self, txtRect, dimLinePt1, dimLinePt2):
      ...
      """
        if txtRect is None:  # se non c'é il testo (é stato spostato fuori dalla linea di quota)
            ...
            partial1SpaceForBlock1, partial1SpaceForBlock2 = self.getSpaceForBlock1AndBlock2OnLineAuxiliary(dimLinePt1,
                                                                                                            dimLinePt2, \
                                                                                                            linearObject.getStartPt())
            linearObject = txtRect.getLinearObjectAt(1)
            partial2SpaceForBlock1, partial2SpaceForBlock2 = self.getSpaceForBlock1AndBlock2OnLineAuxiliary(dimLinePt1,
                                                                                                            dimLinePt2, \
                                                                                                            linearObject.getStartPt())
            ...
            partial3SpaceForBlock1, partial3SpaceForBlock2 = self.getSpaceForBlock1AndBlock2OnLineAuxiliary(dimLinePt1,
                                                                                                            dimLinePt2, \
                                                                                                            linearObject.getStartPt())
            ...
            partial4SpaceForBlock1, partial4SpaceForBlock2 = self.getSpaceForBlock1AndBlock2OnLineAuxiliary(dimLinePt1,
                                                                                                            dimLinePt2, \
                                                                                                            linearObject.getStartPt())
    ...
    def getSymbolFeature(self, insPt, rot, isBlock1, textLinearDimComponentOn, sourceCrs=None):
      ...
      """
        ...
        if self.dimType == QadDimTypeEnum.RADIUS:  # se quotatura tipo raggio
            ...
            try:
                if len(self.componentFieldName) > 0:
                    f.setAttribute(self.componentFieldName,
                                   QadDimComponentEnum.BLOCK1 if isBlock1 else QadDimComponentEnum.BLOCK2)
            except:
                pass
    ...
    def getDimPointFeature(self, insPt, isDimPt1, sourceCrs):
      ...
      """
        ...
        try:
            f.setAttribute(self.componentFieldName,
                           QadDimComponentEnum.DIM_PT1 if isDimPt1 else QadDimComponentEnum.DIM_PT2)
        except:
            pass
    ...
    def getFormattedText(self, measure):
      ...
      """
        if type(measure) == int or type(measure) == float:
            strIntPart, strDecPart = qad_utils.getStrIntDecParts(
                round(measure, self.textDecimals))  # numero di decimali

            if strIntPart == "0" and self.textSuppressLeadingZeros == True:  # per sopprimere o meno gli zero all'inizio del testo
                strIntPart = ""

            for i in xrange(0, self.textDecimals - len(strDecPart),
                            1):  # aggiunge "0" per arrivare al numero di decimali
    ...
    def getBoundingPointsTextRectProjectedToLine(self, pt1, pt2, textRect):
      ...
      """
        ...
        qad_utils.appendUniquePointToList(perpPts,
                                          qad_utils.getPerpendicularPointOnInfinityLine(pt1, pt2, rectCorners[0]))
        qad_utils.appendUniquePointToList(perpPts,
                                          qad_utils.getPerpendicularPointOnInfinityLine(pt1, pt2, rectCorners[1]))
        qad_utils.appendUniquePointToList(perpPts,
                                          qad_utils.getPerpendicularPointOnInfinityLine(pt1, pt2, rectCorners[2]))
        qad_utils.appendUniquePointToList(perpPts,
                                          qad_utils.getPerpendicularPointOnInfinityLine(pt1, pt2, rectCorners[3]))
      ...
      """
        ...
        if rotMode == QadDimStyleTxtRotModeEnum.ALIGNED_LINE:  # testo allineato alla linea
            ...
            if horizontalPos == QadDimStyleTxtHorizontalPosEnum.CENTERED_LINE:  # testo centrato alla linea
                ...
                if textInsPtCloseToPt1:  # il punto di inserimento del testo é vicino a pt1
                    ...
                    insPt = qad_utils.getPolarPointByPtAngle(pt1, lineRot,
                                                             textWidth + self.textOffsetDist + self.textOffsetDist)
                ...
                if textInsPtCloseToPt1:  # il punto di inserimento del testo é vicino a pt1
                    insPt = qad_utils.getPolarPointByPtAngle(pt1, lineRot, lineLen - textWidth - (
                                self.textOffsetDist + self.textOffsetDist))
                else:  # il punto di inserimento del testo é vicino a pt2
                    insPt = qad_utils.getPolarPointByPtAngle(pt1, lineRot,
                                                             lineLen - (self.textOffsetDist + self.textOffsetDist))
            ...
            if verticalPos == QadDimStyleTxtVerticalPosEnum.CENTERED_LINE:  # testo centrato alla linea
                ...
                if textInsPtCloseToPt1:  # il punto di inserimento del testo é vicino a pt1
                    insPt = qad_utils.getPolarPointByPtAngle(insPt, lineRot + math.pi / 2,
                                                             self.textOffsetDist + self.textOffsetDist)
                else:  # il punto di inserimento del testo é vicino a pt2
                    insPt = qad_utils.getPolarPointByPtAngle(insPt, lineRot - math.pi / 2,
                                                             self.textOffsetDist + self.textOffsetDist)
                ...
                if textInsPtCloseToPt1:  # il punto di inserimento del testo é vicino a pt1
                    insPt = qad_utils.getPolarPointByPtAngle(insPt, lineRot - math.pi / 2,
                                                             textHeight + (self.textOffsetDist + self.textOffsetDist))
                else:  # il punto di inserimento del testo é vicino a pt2
                    insPt = qad_utils.getPolarPointByPtAngle(insPt, lineRot + math.pi / 2,
                                                             textHeight + (self.textOffsetDist + self.textOffsetDist))
        ...
        # testo orizzontale o testo con rotazione forzata
            ...
            if horizontalPos == QadDimStyleTxtHorizontalPosEnum.CENTERED_LINE:  # testo centrato alla linea
                ...
                closestPtToPt1 = qad_utils.getPolarPointByPtAngle(pt1, lineRot,
                                                                  lineLen - self.textOffsetDist - projectedTextWidth)
    ...
    def getTextPositionOnArc(self, arc, textWidth, textHeight, horizontalPos, verticalPos, rotMode):
      ...
      """
        ...
        if horizontalPos == QadDimStyleTxtHorizontalPosEnum.CENTERED_LINE:  # testo centrato alla linea
            ...
            # uso 2 volte textOffsetDist perché una volta é la distanza dal punto pt1 + un offset intorno al testo
            insPtCenterTxt = arc.getPtFromStart(
                arcLength - TextWidthOnArc / 2 - textOffsetDistOnArc - textOffsetDistOnArc)
            lineRot = arc.getTanDirectionOnPt(insPtCenterTxt)
        ...
        if verticalPos == QadDimStyleTxtVerticalPosEnum.CENTERED_LINE:  # testo centrato alla linea
            if textRot > (math.pi / 2) and textRot <= math.pi * 3 / 2:  # se il testo é capovolto
                if (
                        angleOnCenterTxt > 0 and angleOnCenterTxt <= math.pi):  # il testo va verso il punto finale dell'arco
                ...
                if (
                        angleOnCenterTxt > 0 and angleOnCenterTxt <= math.pi):  # il testo va verso il punto iniziale dell'arco
                    insPt = qad_utils.getPolarPointByPtAngle(insPt, angleOnCenterTxt, -textHeight / 2)
            ...
            if textRot > (math.pi / 2) and textRot <= math.pi * 3 / 2:  # se il testo é capovolto
                if (
                        angleOnCenterTxt > 0 and angleOnCenterTxt <= math.pi):  # il testo va verso il punto finale dell'arco
                    insPt = qad_utils.getPolarPointByPtAngle(insPt, angleOnCenterTxt,
                                                             (textHeight + self.textOffsetDist))
                else:  # il testo va verso il punto iniziale dell'arco
                    insPt = qad_utils.getPolarPointByPtAngle(insPt, angleOnCenterTxt,
                                                             -(textHeight + self.textOffsetDist))
            else:  # il testo è dritto
    ...
    def getLinearTextAndBlocksPosition(self, dimPt1, dimPt2, dimLinePt1, dimLinePt2, textWidth, textHeight):
      ...
      """
        ...
        if self.textHorizontalPos == QadDimStyleTxtHorizontalPosEnum.CENTERED_LINE or \
            ...
            # e il testo sia dentro le linee di estensione e forzato orizzontale allora il testo diventa centrato
            if (
                    self.textVerticalPos == QadDimStyleTxtVerticalPosEnum.ABOVE_LINE or self.textVerticalPos == QadDimStyleTxtVerticalPosEnum.BELOW_LINE) and \
                    (
            ...
            if self.textRotMode == QadDimStyleTxtRotModeEnum.ISO:
                textInsPt, textRot = self.getTextPositionOnLine(dimLinePt1Offset, dimLinePt2Offset, textWidth,
                                                                textHeight, \
                                                                self.textHorizontalPos, textVerticalPos,
            ...
            if spaceForBlock1 == 0 or spaceForBlock2 == 0 or \
                ...
                if self.blockSuppressionForNoSpace:  # sopprime i simboli se non c'é spazio sufficiente all'interno delle linee di estensione
                    ...
                    if spaceForBlock1 == 0 or spaceForBlock2 == 0 or \
                        ...
                        textInsPt, textRot, txtLeaderLines = self.getTextPosAndLinesOutOfDimLines(dimLinePt1,
                                                                                                  dimLinePt2, textWidth,
                                                                                                  textHeight)
                    ...
                    # sposta prima le frecce poi, se non basta, anche il testo
                        ...
                        # considero il testo senza frecce
                        if self.textRotMode == QadDimStyleTxtRotModeEnum.ISO:
                            textInsPt, textRot = self.getTextPositionOnLine(dimLinePt1, dimLinePt2, textWidth,
                                                                            textHeight, \
                                                                            self.textHorizontalPos, textVerticalPos,
                                                                            QadDimStyleTxtRotModeEnum.ALIGNED_LINE)
                        else:
                        ...
                        rect = self.textRectToQadLinearObjectList(textInsPt, textWidth, textHeight, textRot)
                        spaceForBlock1, spaceForBlock2 = self.getSpaceForBlock1AndBlock2OnLine(rect, dimLinePt1,
                                                                                               dimLinePt2)
                        # se non c'é spazio neanche per il testo senza le frecce
                        if spaceForBlock1 == 0 or spaceForBlock2 == 0 or \
                                spaceForBlock1 < self.textOffsetDist or spaceForBlock2 < self.textOffsetDist:
                        ...
                        if qad_utils.getDistance(dimLinePt1, dimLinePt2) <= self.getBlock1Size() + self.getBlock2Size():
                            block1Rot, block2Rot = self.getBlocksRotOnLine(dimLinePt1, dimLinePt2,
                                                                           False)  # frecce esterne
                        else:
                            block1Rot, block2Rot = self.getBlocksRotOnLine(dimLinePt1, dimLinePt2,
                                                                           True)  # frecce interne
                    # Sposta indistintamente il testo o le frecce (l'oggetto che si adatta meglio)
                        ...
                        if self.getBlock1Size() + self.getBlock2Size() > textWidth:  # le frecce sono più ingombranti del testo
                            ...
                            textLinearDimComponentOn = QadDimComponentEnum.LEADER_LINE
                            # se non ci stanno neanche le frecce
                            if qad_utils.getDistance(dimLinePt1,
                                                     dimLinePt2) <= self.getBlock1Size() + self.getBlock2Size():
                                block1Rot, block2Rot = self.getBlocksRotOnLine(dimLinePt1, dimLinePt2,
                                                                               False)  # frecce esterne
                            else:
                                block1Rot, block2Rot = self.getBlocksRotOnLine(dimLinePt1, dimLinePt2,
        ...
        if self.textHorizontalPos == QadDimStyleTxtHorizontalPosEnum.CENTERED_LINE or \
            ...
            if self.textVerticalPos == QadDimStyleTxtVerticalPosEnum.EXTERN_LINE:
                ...
                if dimCenterToTextInsPt_rot > 0 and \
                        (dimCenterToTextInsPt_rot <= math.pi or qad_utils.doubleNear(dimCenterToTextInsPt_rot,
                                                                                     math.pi)):
            ...
            if spaceForBlock1 == 0 or spaceForBlock2 == 0 or \
                ...
                if self.blockSuppressionForNoSpace:  # sopprime i simboli se non c'é spazio sufficiente all'interno delle linee di estensione
                    ...
                    if spaceForBlock1 == 0 or spaceForBlock2 == 0 or \
                            spaceForBlock1 < self.textOffsetDist or spaceForBlock2 < self.textOffsetDist:
                        # sposta testo fuori dalle linee di estensione
                        textInsPt, textRot, txtLeaderLines = self.getTextPosAndLinesOutOfDimArc(dimLineArc, textWidth,
                                                                                                textHeight)
                        textLinearDimComponentOn = QadDimComponentEnum.LEADER_LINE
                    else:
                        textLinearDimComponentOn = QadDimComponentEnum.DIM_LINE1
                    ...
                    # sposta prima le frecce poi, se non basta, anche il testo
                        ...
                        if spaceForBlock1 == 0 or spaceForBlock2 == 0 or \
                                spaceForBlock1 < self.textOffsetDist or spaceForBlock2 < self.textOffsetDist:
                            # sposta testo fuori dalle linee di estensione
                            textInsPt, textRot, txtLeaderLines = self.getTextPosAndLinesOutOfDimArc(dimLineArc,
                                                                                                    textWidth,
                                                                                                    textHeight)
                            textLinearDimComponentOn = QadDimComponentEnum.LEADER_LINE
                        else:
    ...
    def getLeaderLinesOnArc(self, dimLineArc, textWidth, textHeight):
      ...
      """
        ...
        if self.textHorizontalPos == QadDimStyleTxtHorizontalPosEnum.FIRST_EXT_LINE:
            startPt = dimLineArc.getStartPt()
            rotLine = dimLineArc.getTanDirectionOnPt(
                startPt) + math.pi  # angolo della linea porta quota sul punto iniziale
    ...
    def getExtLineFeature(self, extLine, isExtLine1, sourceCrs=None):
      ...
      """
        ...
        try:
            ...
            if len(self.componentFieldName) > 0:
                f.setAttribute(self.componentFieldName,
                               QadDimComponentEnum.EXT_LINE1 if isExtLine1 else QadDimComponentEnum.EXT_LINE2)
        except:
            pass
    ...
    def getDimLine(self, dimPt1, dimPt2, linePosPt, preferredAlignment=QadDimStyleAlignmentEnum.HORIZONTAL,
      ...
      """
        if self.dimType == QadDimTypeEnum.ALIGNED:
            ...
            if qad_utils.leftOfLine(linePosPt, dimPt1,
                                    dimPt2) < 0:  # a sinistra della linea che congiunge <dimPt1> a <dimPt2>
    ...
    def getDimLineFeature(self, dimLine, isDimLine1, textLinearDimComponentOn, sourceCrs=None):
      ...
      """
        ...
        try:
            ...
            if len(self.componentFieldName) > 0:
                f.setAttribute(self.componentFieldName,
                               QadDimComponentEnum.DIM_LINE1 if isDimLine1 else QadDimComponentEnum.DIM_LINE2)
        except:
            pass
    ...
    def getLinearDimFeatures(self, canvas, dimPt1, dimPt2, linePosPt, measure=None, \
      ...
      """
        ...
        # blocchi frecce
        block1Feature = self.getSymbolFeature(dimLine1[0], block1Rot, True, textLinearDimComponentOn,
                                              canvas.mapSettings().destinationCrs())  # True = primo punto di quotatura
        block2Feature = self.getSymbolFeature(dimLine1[1], block2Rot, False, textLinearDimComponentOn,
                                              canvas.mapSettings().destinationCrs())  # False = secondo punto di quotatura
        ...
        textOffsetRect = self.textRectToQadLinearObjectList(textOffsetRectInsPt, textWidthOffset, textHeightOffset,
                                                            textRot)

        if textLinearDimComponentOn == QadDimComponentEnum.DIM_LINE1:  # linea di quota ("Dimension line")
            dimLine1, dimLine2 = self.adjustLineAccordingTextRect(textOffsetRect, dimLine1[0], dimLine1[1],
                                                                  QadDimComponentEnum.DIM_LINE1)
        elif textLinearDimComponentOn == QadDimComponentEnum.EXT_LINE1:  # prima linea di estensione ("Extension line 1")
            if extLine1 is not None:
                extLineRot = qad_utils.getAngleBy2Pts(dimPt1, dimLine1[0])
                extLine1 = self.getExtLine(dimPt1, qad_utils.getPolarPointByPtAngle(dimLine1[0], extLineRot,
                                                                                    textWidth + self.textOffsetDist))
                ...
                extLine1, dummy = self.adjustLineAccordingTextRect(textOffsetRect, extLine1[1], extLine1[0],
                                                                   QadDimComponentEnum.EXT_LINE1)
        elif textLinearDimComponentOn == QadDimComponentEnum.EXT_LINE2:  # seconda linea di estensione ("Extension line 2")
            if extLine2 is not None:
                extLineRot = qad_utils.getAngleBy2Pts(dimPt2, dimLine1[1])
                extLine2 = self.getExtLine(dimPt2, qad_utils.getPolarPointByPtAngle(dimLine1[1], extLineRot,
                                                                                    textWidth + self.textOffsetDist))
                # passo prima il secondo punto e poi il primo perché getExtLine restituisce una linea dalla linea di quota verso il punto di quotatura
                extLine2, dummy = self.adjustLineAccordingTextRect(textOffsetRect, extLine2[1], extLine2[0],
                                                                   QadDimComponentEnum.EXT_LINE2)
        elif textLinearDimComponentOn == QadDimComponentEnum.LEADER_LINE:  # linea porta quota usata quando il testo é fuori dalla quota ("Leader")
            lastLine = txtLeaderLines[-1]
            lastLine, dummy = self.adjustLineAccordingTextRect(textOffsetRect, lastLine[0], lastLine[1],
                                                               QadDimComponentEnum.LEADER_LINE)
            del txtLeaderLines[-1]  # sostituisco l'ultimo elemento
            txtLeaderLines.append(lastLine)

        # linee di quota
        dimLine1Feature = self.getDimLineFeature(dimLine1, True, textLinearDimComponentOn,
                                                 canvas.mapSettings().destinationCrs())  # True = prima linea di quota
        dimLine2Feature = self.getDimLineFeature(dimLine2, False, textLinearDimComponentOn,
                                                 canvas.mapSettings().destinationCrs())  # False = seconda linea di quota

        # linee di estensione
        extLine1Feature = self.getExtLineFeature(extLine1, True,
                                                 canvas.mapSettings().destinationCrs())  # True = prima linea di estensione
        extLine2Feature = self.getExtLineFeature(extLine2, False,
                                                 canvas.mapSettings().destinationCrs())  # False = seconda linea di estensione
    ...
    def getArcDimFeatures(self, canvas, dimArc, linePosPt, measure=None, arcLeader=None):
      ...
      """
        ...
        # trovo il vero punto di inserimento del testo tenendo conto del buffer intorno
        if self.arcSymbPos == QadDimStyleArcSymbolPosEnum.BEFORE_TEXT:
            textInsPt = qad_utils.getPolarPointByPtAngle(textOffsetRectInsPt, textRot,
                                                         self.textOffsetDist + self.textOffsetDist + 2 * arcSymbRadius)
            textInsPt = qad_utils.getPolarPointByPtAngle(textInsPt, textRot + math.pi / 2, self.textOffsetDist)
        else:
            textInsPt = qad_utils.getPolarPointByPtAngle(textOffsetRectInsPt, textRot, self.textOffsetDist)
            textInsPt = qad_utils.getPolarPointByPtAngle(textInsPt, textRot + math.pi / 2, self.textOffsetDist)
        ...
        # blocchi frecce
        block1Feature = self.getSymbolFeature(dimLineArc1StartPt, block1Rot, True, textLinearDimComponentOn,
                                              canvas.mapSettings().destinationCrs())  # True = primo punto di quotatura
        block2Feature = self.getSymbolFeature(dimLineArc1EndPt, block2Rot, False, textLinearDimComponentOn,
                                              canvas.mapSettings().destinationCrs())  # False = secondo punto di quotatura
        ...
        if textLinearDimComponentOn == QadDimComponentEnum.DIM_LINE1:  # linea di quota ("Dimension line")
            dimLineArc1, dimLineArc2 = self.adjustArcAccordingTextRect(textOffsetRect, dimLineArc1,
                                                                       QadDimComponentEnum.DIM_LINE1)
        elif textLinearDimComponentOn == QadDimComponentEnum.EXT_LINE1:  # prima linea di estensione ("Extension line 1")
            if extLine1 is not None:
                extLineRot = qad_utils.getAngleBy2Pts(dimPt1, dimLineArc1StartPt)
                extLine1 = self.getExtLine(dimPt1, qad_utils.getPolarPointByPtAngle(dimLineArc1StartPt, extLineRot,
                                                                                    textWidth + self.textOffsetDist))
                # passo prima il secondo punto e poi il primo perché getExtLine restituisce una linea dalla linea di quota verso il punto di quotatura
                extLine1, dummy = self.adjustLineAccordingTextRect(textOffsetRect, extLine1[1], extLine1[0],
                                                                   QadDimComponentEnum.EXT_LINE1)
            ...
            if extLine2 is not None:
                ...
                extLine2 = self.getExtLine(dimPt2, qad_utils.getPolarPointByPtAngle(dimLineArc1EndPt, extLineRot,
                                                                                    textWidth + self.textOffsetDist))
                # passo prima il secondo punto e poi il primo perché getExtLine restituisce una linea dalla linea di quota verso il punto di quotatura
                extLine2, dummy = self.adjustLineAccordingTextRect(textOffsetRect, extLine2[1], extLine2[0],
                                                                   QadDimComponentEnum.EXT_LINE2)
        ...
        if dimLineArc1 is None:
            ...
            dimLine1Feature = self.getDimLineFeature(dimLineArc1.asPolyline(), True, textLinearDimComponentOn,
                                                     canvas.mapSettings().destinationCrs())  # True = prima linea di quota

        if dimLineArc2 is None:
            dimLine2Feature = None
        else:
            dimLine2Feature = self.getDimLineFeature(dimLineArc2.asPolyline(), False, textLinearDimComponentOn,
                                                     canvas.mapSettings().destinationCrs())  # False = seconda linea di quota
        ...
        if arcLeader:  # se si vuole la linea che congiunge il testo all'arco da quotare
            ...
            if arcLeaderLines is not None:
                ...
                arcLeaderBlockFeature = self.getLeaderSymbolFeature(arcLeaderLines[1],
                                                                    qad_utils.getAngleBy2Pts(arcLeaderLines[0],
                                                                                             arcLeaderLines[1]), \
                                                                    canvas.mapSettings().destinationCrs())
        # simbolo dell'arco
        ...
        if self.arcSymbPos == QadDimStyleArcSymbolPosEnum.BEFORE_TEXT:
            ...
            arcCenter = qad_utils.getPolarPointByPtAngle(arcCenter, textRot + math.pi / 2,
                                                         arcSymbRadius + self.textOffsetDist)
...
class QadDimEntity():
    ...
    def addToLayers(self, plugIn):
        ...
        # plugIn, layer, feature, coordTransform, refresh, check_validity
        if qad_layer.addFeatureToLayer(plugIn, self.getTextualLayer(), self.textualFeature, None, False,
                                       False) == False:
            return False
        ...
        if qad_layer.updateFeatureToLayer(plugIn, self.getTextualLayer(), self.textualFeature, False, False) == False:
            return False
        # plugIn, layer, features, coordTransform, refresh, check_validity
        if qad_layer.addFeaturesToLayer(plugIn, self.getLinearLayer(), self.linearFeatures, None, False,
                                        False) == False:
            return False
        # plugIn, layer, features, coordTransform, refresh, check_validity
        if qad_layer.addFeaturesToLayer(plugIn, self.getSymbolLayer(), self.symbolFeatures, None, False,
                                        False) == False:
            return False
                    ...
                    if value == QadDimComponentEnum.DIM_PT1:  # primo punto da quotare ("Dimension point 1")
                        ...
                        if (destinationCrs is not None) and destinationCrs != self.getSymbolLayer().crs():
                            g.transform(QgsCoordinateTransform(self.getSymbolLayer().crs(),
                                                               destinationCrs))  # trasformo la geometria in map coordinate
    ...
    def move(self, offSetX, offSetY):
        ...
        if (destinationCrs is not None) and destinationCrs != self.getTextualLayer().crs():
            g.transform(QgsCoordinateTransform(destinationCrs,
                                               self.getTextualLayer().crs()))  # trasformo la geometria in layer coordinate
        ...
        for f in self.linearFeatures:
            ...
            if (destinationCrs is not None) and destinationCrs != self.getLinearLayer().crs():
                g.transform(QgsCoordinateTransform(destinationCrs,
                                                   self.getLinearLayer().crs()))  # trasformo la geometria in layer coordinate
        ...
        for f in self.symbolFeatures:
            ...
            if (destinationCrs is not None) and destinationCrs != self.getSymbolLayer().crs():
                g.transform(QgsCoordinateTransform(destinationCrs,
                                                   self.getSymbolLayer().crs()))  # trasformo la geometria in layer coordinate
      ...
      """
        ...
        if self.dimStyle.dimType == QadDimTypeEnum.ALIGNED:  # quota lineare allineata ai punti di origine delle linee di estensione
            ...
            if linePosPt is not None:
                ...
                linePosPt = qad_utils.getPolarPointBy2Pts(dimArc.center, linePosPt,
                                                          qad_utils.getDistance(dimArc.center, newPt))
            else:
                linePosPt = self.getDimLinePosPt(None, destinationCrs)
                # verifico se è stato coinvolto il testo della quota
