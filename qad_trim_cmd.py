...
class QadTRIMCommandClass(QadCommandClass):
    ...
    def connectQAction(self, action):
        action.triggered = self.plugIn.runTRIMCommand
    ...
    def trimFeatures(self, geom, toExtend):
        ...
        for layerEntitySet in self.entitySet.layerEntitySetList:
            ...
            for featureId in layerEntitySet.featureIds:
                ...
                for intPt in intPts:
                    if toExtend:
                        newGeom = qad_utils.extendQgsGeometry(self.plugIn.canvas.mapSettings().destinationCrs(), f_geom,
                                                              intPt, \
                        ...
                        if newGeom is not None:
                            ...
                            if qad_layer.updateFeatureToLayer(self.plugIn, layer, extendedFeature, False,
                                                              False) == False:
                                self.plugIn.destroyEditCommand()
                                return
                    else:  # trim
                        result = qad_utils.trimQgsGeometry(self.plugIn.canvas.mapSettings().destinationCrs(), f_geom,
                                                           intPt, \
                                                           self.limitEntitySet, self.edgeMode, \
                                                           tolerance2ApproxCurve)
                        if result is not None:
                            ...
                            if layer.geometryType() == QGis.Line:
                                ...
                                # plugIn, layer, feature, refresh, check_validity
                                if qad_layer.updateFeatureToLayer(self.plugIn, layer, trimmedFeature1, False,
                                                                  False) == False:
                                    self.plugIn.destroyEditCommand()
                                    return
                                if line2 is not None:
                                    ...
                                    if qad_layer.addFeatureToLayer(self.plugIn, layer, trimmedFeature2, None, False,
                                                                   False) == False:
                                        self.plugIn.destroyEditCommand()
                                        return
                            else:
                                ...
                                if qad_layer.addGeometriesToQADTempLayers(self.plugIn, None, lineGeoms, None, None,
                                                                          False) == False:
                                    self.plugIn.destroyEditCommand()
                                    return
    ...
    def waitForObjectSel(self):
        ...
        prompt = QadMsg.translate("Command_TRIM",
                                  "Select the object to trim or shift-select to extend or [{0}]: ").format(keyWords)
        ...
        # RISPOSTA ALLA SELEZIONE OGGETTI DA ESTENDERE
            ...
            if type(value) == unicode:
                if value == QadMsg.translate("Command_TRIM", "Fence") or value == "Fence":
                    ...
                    prompt = QadMsg.translate("Command_TRIM", "Specify an extension mode [{0}] <{1}>: ").format(
