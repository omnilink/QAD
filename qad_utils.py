...
def getVisibleVectorLayers(canvas):
    ...
    for i in xrange(len(layers) - 1, -1, -1):
        ...
        if layers[i].type() != QgsMapLayer.VectorLayer or \
                ...
                (canvas.mapSettings().scale() < layers[i].minimumScale() or canvas.mapSettings().scale() > layers[
                    i].maximumScale()):
            del layers[i]
    return layers
...
def getGeomInBox(point, mQgsMapTool, geoms, boxSize, crs=None, \
   ...
   """
    if geoms is None:
        return False
    for geom in geoms:
        if isGeomInBox(point, mQgsMapTool, geom, boxSize, crs, checkPointLayer, checkLineLayer, checkPolygonLayer,
                       onlyBoundary):
            return geom
    return None
            ...
            if mode.upper() == "X":  # take all features
                ...
                if wkbType == QGis.WKBPoint or wkbType == QGis.WKBPoint25D:
                    Tolerance = QadVariables.get(
                        QadMsg.translate("Environment variables", "PICKBOX"))  # leggo la tolleranza
    ...
    if partList1ToTrim is not None:
        ...
        if newPt1 is None:  # nessuna intersezione
            if LinearObjectListToCut.isClosed():  # se é chiusa
                ...
                newPt, partNumberAtpartList = partList2ToTrim.getIntPtNearestToStartPt(geom_crs, limitEntitySet,
                                                                                       edgeMode)
                if newPt is None:
                    return None
        ...
        if newPt2 is None:  # nessuna intersezione
            if LinearObjectListToCut.isClosed():  # se é chiusa
                if partList1ToTrim is None:
                    return None
                newPt, partNumberAtpartList = partList1ToTrim.getIntPtNearestToStartPt(geom_crs, limitEntitySet,
                                                                                       edgeMode)
                if newPt is None:
                    return None
                for i in xrange(0, partNumberAtpartList, 1):
        ...
        if LinearObjectListToCut.isClosed():  # se é chiusa
            ...
            if partList2ToTrim.qty() > 0:
                ...
                if circle is not None:  # se era una cerchio
                    ...
                    arc.fromStartSecondEndPts(linearObject.getStartPt(), linearObject.getEndPt(),
                                              partList2ToTrim.getEndPt())
   ...
   """
    if gapType == 0:  # gli spigoli del rettangolo hanno angoli retti
        ...
        if gapType == 1:  # raccorda gli spigoli del rettangolo con un raggio di curvatura gapValue1
            ...
            widthDim = (area + angleArea) / lengthDim
            if (gapValue1 + gapValue2) > lengthDim or (
                    gapValue1 + gapValue2) > widthDim:  # il rettangolo é troppo piccolo
                widthDim = area / lengthDim
            return getRectByCornerAndDims(firstCorner, lengthDim, widthDim, rot, gapType, \
                                          gapValue1, gapValue2, tolerance2ApproxCurve)
   ...
   """
        ...
        if part.isSegment():  # segmento
                    ...
                    if IntPointTypeForPart == 1:  # TIP
                        ...
                        if IntPointTypeForPart == 3:  # PFIP
                            if IntPointTypeForNextPart == 2:  # FIP
                                untrimmedOffsetPartList.append([lastUntrimmedOffsetPt, part.getEndPt()])
                                untrimmedOffsetPartList.append(
                                    fillet2Parts_offset(part, nextPart, offSetSide, offSetDist))
                        else:  # NFIP
                            if IntPointTypeForNextPart == 1:  # TIP
                                untrimmedOffsetPartList.append([lastUntrimmedOffsetPt, part.getEndPt()])
                                untrimmedOffsetPartList.append([part.getEndPt(), nextPart.getStartPt()])
            ...
            if nextPart.isSegment():  # arco-segmento
                ...
                if IntPointInfo is not None:  # se esiste un punto di intersezione
                    ...
                    minDistArc, maxDistArc = getOffsetDistancesFromCenterOnOffsetedArc(arc, inverse, offSetDist,
                                                                                       offSetSide)
                    minDistNextArc, maxDistNextArc = getOffsetDistancesFromCenterOnOffsetedArc(nextArc, nextInverse,
                                                                                               offSetDist, offSetSide)
    ...
    if newPartList.qty() > 0:
        if isClosedPolyline == False:
            ...
            untrimmedOffsetPartList.getLinearObjectAt(0).setStartPt(
                untrimmedOffsetPartList.getLinearObjectAt(-1).getEndPt())  # modifico l'inizio
...
def offSetPolyline(points, epsg, offSetDist, offSetSide, gapType, tolerance2ApproxCurve=None):
   ...
   """
    ...
    untrimmedReversedOffsetPartList = getUntrimmedOffSetPartList(reversedPartList, offSetDist, offSetSide, gapType,
                                                                 tolerance)
...
def ApproxCurvesOnGeom(geom, atLeastNSegmentForArc=None, atLeastNSegmentForCircle=None,
   ...
   """
    ...
    if atLeastNSegmentForCircle is None:
        _atLeastNSegmentForCircle = QadVariables.get(QadMsg.translate("Environment variables", "CIRCLEMINSEGMENTQTY"),
                                                     12)
    else:
    ...
    def getIntersectionPtsWithLinearObject(self, linearObject):
      ...
      """
        ...
        if self.isSegment():  # segmento
            ...
            if linearObject.isSegment():  # segmento
                return self.getArc().getIntersectionPointsWithSegment(linearObject.getStartPt(),
                                                                      linearObject.getEndPt())
            else:  # arco
    ...
    def selfJoin(self, epsg):
      ...
      """
        ...
        for featureIdToJoin in idList:
            ...
            joinFeatureInVectorLayer(featureIdToJoin, vectorLayer, QadVariables.get(
