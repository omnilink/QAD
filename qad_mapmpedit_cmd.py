...
class QadMAPMPEDITCommandClass(QadCommandClass):
    ...
    def addEntitySetToPolygon(self, entitySet, removeOriginals=False):
        ...
        if removeOriginals:
            for layerEntitySet in entitySet.layerEntitySetList:
                if qad_layer.deleteFeaturesToLayer(self.plugIn, layerEntitySet.layer, layerEntitySet.featureIds,
                                                   False) == False:
                    self.plugIn.destroyEditCommand()
                    return
    ...
    def delCurrentSubGeomToPolygon(self):
        ...
        if len(self.currAtSubGeom) == 2:
            ...
            if geom.deleteRing(ring + 1,
                               part) == False:  # cancello una isola (Ring 0 is outer ring and can't be deleted)
                self.showMsg(QadMsg.translate("QAD", "Invalid object."))
                return False
        else:
    ...
    def splitPolygon(self, splitLine, createNewEntities):
        ...
        if len(newfeatures) > 0:
            ...
            if qad_layer.addFeaturesToLayer(self.plugIn, self.poligonEntity.layer, newfeatures, None, False,
                                            False) == False:
                self.plugIn.destroyEditCommand()
                return
    ...
    def run(self, msgMapTool=False, msg=None):
        ...
        # RISPOSTA ALLA RICHIESTA DELLA MODALITA' DI UNION (da step = 2)
            ...
            if self.SSGetClass.run(msgMapTool, msg) == True:
                if self.SSGetClass.entitySet.count() > 0:
                    self.unionIntersSubtractEntitySetToPolygon(self.SSGetClass.entitySet,
                                                               QadMAPMPEDITCommandOpTypeEnum.UNION)
                self.reinitSSGetClass()
                self.WaitForMainMenu()
            return False  # continua
        ...
        # RISPOSTA ALLA RICHIESTA DELLA MODALITA' DI INTERSECT (da step = 2)
            ...
            if self.SSGetClass.run(msgMapTool, msg) == True:
                if self.SSGetClass.entitySet.count() > 0:
                    self.unionIntersSubtractEntitySetToPolygon(self.SSGetClass.entitySet,
                                                               QadMAPMPEDITCommandOpTypeEnum.INTERSECTION)
                self.reinitSSGetClass()
                self.WaitForMainMenu()
            return False  # continua
