...
from qad.qad_dimstyle_new_dlg import QadDIMSTYLE_NEW_Dialog
from qad.qad_dimstyle_details_dlg import QadDIMSTYLE_DETAILS_Dialog, QadPreviewDim
from qad.qad_dimstyle_diff_dlg import QadDIMSTYLE_DIFF_Dialog
...
class QadDIMSTYLEDialog(QDialog, QObject, Ui_DimStyle_Dialog):
    ...
    def updDescrSelectedDimStyle(self):
        if self.selectedDimStyle is None:
            return
        Title = QadMsg.translate("DimStyle_Dialog",
                                 "QAD - Editing dimension style description: ") + self.selectedDimStyle.name
    ...
    def createNewStyle(self):
        ...
        Form = QadDIMSTYLE_NEW_Dialog(self.plugIn, self,
                                      self.selectedDimStyle.name if self.selectedDimStyle is not None else None)
    ...
    def temporaryModStyle(self):
        ...
        Form = QadDIMSTYLE_DETAILS_Dialog(self.plugIn, self, self.selectedDimStyle)
        title = QadMsg.translate("DimStyle_Dialog",
                                 "Set temporary overrides to dimension style: ") + self.selectedDimStyle.name
        Form.setWindowTitle(title)
        if Form.exec_() == QDialog.Accepted:
            self.selectedDimStyle.set(Form.dimStyle)
            self.init()
