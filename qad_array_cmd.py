...
from qad.qad_array_maptool import *
...
from qad.qad_getangle_cmd import QadGetAngleClass
...
class QadARRAYCommandClass(QadCommandClass):
    ...
    def getNote(self):
        ...
        return QadMsg.translate("Command_ARRAY",
                                "Creates copies of objects in a regularly spaced rectangular, polar, or path array.")
        ...
        for layerEntitySet in entitySet.layerEntitySetList:
            ...
            while len(layerEntitySet.featureIds) > 0:
                ...
                if dimEntity is None:
                    ...
                    if qad_array_fun.arrayRectangleEntity(self.plugIn, entity, self.basePt, self.rectangleRows,
                                                          self.rectangleCols, \
                                                          self.distanceBetweenRows, self.distanceBetweenCols,
                                                          self.rectangleAngle, self.itemsRotation,
                                                          True, None) == False:
                        self.plugIn.destroyEditCommand()
                        return
                    del layerEntitySet.featureIds[0]  # la rimuovo da entitySet
                else:
                    if qad_array_fun.arrayRectangleEntity(self.plugIn, dimEntity, self.basePt, self.rectangleRows,
                                                          self.rectangleCols, \
                                                          self.distanceBetweenRows, self.distanceBetweenCols,
                                                          self.rectangleAngle, self.itemsRotation,
                                                          True, None) == False:
    ...
    def doPathArray(self):
        ...
        for layerEntitySet in entitySet.layerEntitySetList:
            ...
            while len(layerEntitySet.featureIds) > 0:
                ...
                if dimEntity is None:
                    entity.set(layer, f.id())
                    if qad_array_fun.arrayPathEntity(self.plugIn, entity, self.basePt, self.pathRows,
                                                     self.pathItemsNumber, \
                                                     self.distanceBetweenRows, self.distanceBetweenCols,
                                                     self.pathTangentDirection, self.itemsRotation, \
                                                     self.pathLinearObjectList, self.distanceFromStartPt, True,
                                                     None) == False:
                        self.plugIn.destroyEditCommand()
                        return
                    del layerEntitySet.featureIds[0]  # la rimuovo da entitySet
                else:
                    if qad_array_fun.arrayPathEntity(self.plugIn, dimEntity, self.basePt, self.pathRows,
                                                     self.pathItemsNumber, \
                                                     self.distanceBetweenRows, self.distanceBetweenCols,
                                                     self.pathTangentDirection, self.itemsRotation, \
                                                     self.pathLinearObjectList, self.distanceFromStartPt, True,
    ...
    def doPolarArray(self):
        ...
        for layerEntitySet in entitySet.layerEntitySetList:
            ...
            while len(layerEntitySet.featureIds) > 0:
                ...
                if dimEntity is None:
                    entity.set(layer, f.id())
                    if qad_array_fun.arrayPolarEntity(self.plugIn, entity, self.basePt, self.centerPt,
                                                      self.polarItemsNumber, \
                                                      self.polarAngleBetween, self.polarRows, self.distanceBetweenRows,
                                                      self.itemsRotation, \
                                                      True, None) == False:
                        self.plugIn.destroyEditCommand()
                        return
                    ...
                    if qad_array_fun.arrayPolarEntity(self.plugIn, dimEntity, self.basePt, self.centerPt,
                                                      self.polarItemsNumber, \
                                                      self.polarAngleBetween, self.polarRows, self.distanceBetweenRows,
                                                      self.itemsRotation, \
                                                      True, None) == False:
    ...
    def waitForItemsNumber(self):
        ...
        if self.arrayType == QadARRAYCommandClassSeriesTypeEnum.PATH:
            ...
            prompt = QadMsg.translate("Command_ARRAY", "Number of Items to Array or [{0}] <{1}>: ").format(keyWords,
                                                                                                           str(
                                                                                                               self.defaultValue))
            keyWords += "_" + englishKeyWords
            ...
            prompt = QadMsg.translate("Command_ARRAY", "Number of Items to Array <{0}>: ").format(
    ...
    def waitForDistanceBetweenRows(self, totalOption, nextStep):
        ...
        if totalOption:
            ...
            prompt = QadMsg.translate("Command_ARRAY", "Specify distance between rows or [{0}] <{1}>: ").format(
                keyWords, str(self.defaultValue))
            keyWords += "_" + englishKeyWords
            inputType = QadInputTypeEnum.FLOAT | QadInputTypeEnum.POINT2D | QadInputTypeEnum.KEYWORDS
        else:
            prompt = QadMsg.translate("Command_ARRAY", "Specify distance between rows <{0}>: ").format(
                str(self.defaultValue))
            keyWords = ""
    ...
    def waitForDelOrigObjs(self):
        ...
        prompt = QadMsg.translate("Command_ARRAY", "Delete source objects of the array ? [{0}] <{1}>: ").format(
    ...
    def waitForItemsRotation(self):
        ...
        if self.arrayType == QadARRAYCommandClassSeriesTypeEnum.RECTANGLE:
            prompt = QadMsg.translate("Command_ARRAY", "Rotate objects as they are arrayed ? [{0}] <{1}>: ").format(
                keyWords, self.defaultValue)
        elif self.arrayType == QadARRAYCommandClassSeriesTypeEnum.PATH:
            prompt = QadMsg.translate("Command_ARRAY", "Align arrayed items to the path ? [{0}] <{1}>: ").format(
                keyWords, self.defaultValue)
        elif self.arrayType == QadARRAYCommandClassSeriesTypeEnum.POLAR:
            prompt = QadMsg.translate("Command_ARRAY", "Rotate objects as they are arrayed ? [{0}] <{1}>: ").format(
                keyWords, self.defaultValue)
    ...
    def waitForRectangleArrayOptions(self):
        ...
        prompt = QadMsg.translate("Command_ARRAY", "Select an option to edit array or [{0}] <{1}>: ").format(keyWords,
                                                                                                             self.defaultValue)
        keyWords += "_" + englishKeyWords
        # si appresta ad attendere un punto o enter o una parola chiave
        # msg, inputType, default, keyWords, nessun controllo
    ...
    def waitForRectangleTotalDistanceCols(self):
        ...
        if self.GetDistClass is not None:
            del self.GetDistClass
        self.GetDistClass = QadGetDistClass(self.plugIn)
        prompt = QadMsg.translate("Command_ARRAY",
                                  "Specifies the total distance between the start and end columns <{0}>: ")
    ...
    def waitForRectangleDistanceBetweenCols(self, totalOption):
        ...
        if totalOption:
            keyWords = QadMsg.translate("Command_ARRAY", "Total")
            englishKeyWords = "Total"
            prompt = QadMsg.translate("Command_ARRAY", "Specify distance between columns or [{0}] <{1}>: ").format(
                keyWords, str(self.defaultValue))
            keyWords += "_" + englishKeyWords
            inputType = QadInputTypeEnum.FLOAT | QadInputTypeEnum.POINT2D | QadInputTypeEnum.KEYWORDS
        else:
            prompt = QadMsg.translate("Command_ARRAY", "Specify distance between columns <{0}>: ").format(
                str(self.defaultValue))
            keyWords = ""
            inputType = QadInputTypeEnum.FLOAT | QadInputTypeEnum.POINT2D
    ...
    def waitForPathMethod(self):
        ...
        prompt = QadMsg.translate("Command_ARRAY", "Specify path method [{0}] <{1}>: ").format(keyWords,
                                                                                               self.defaultValue)
    ...
    def run(self, msgMapTool=False, msg=None):
            ...
            if type(value) == unicode:
                if self.arrayType == QadARRAYCommandClassSeriesTypeEnum.RECTANGLE:
                    ...
                    if value == QadMsg.translate("Command_ARRAY", "Method") or value == "Method":
                        ...
                        self.waitForBasePt()
                    elif value == QadMsg.translate("Command_ARRAY",
                                                   "Tangent direction") or value == "Tangent direction":
        ...
        # RISPOSTA ALLA RICHIESTA DELLA DISTANZA TRA COLONNE (da step = ASK_FOR_MAIN_OPTIONS)
            ...
            if type(value) == QgsPoint:  # se é stato inserito il primo punto per misurare la distanza tra colonne
                ...
                self.updatePointMapToolParams()
                self.waitForDistanceBetweenRows(False,
                                                QadARRAYCommandClassStepEnum.ASK_FOR_ROW_SPACE)  # senza opzione di "totale"
        ...
        # =========================================================================
            ...
            self.updatePointMapToolParams()
            self.waitForDistanceBetweenRows(True,
                                            QadARRAYCommandClassStepEnum.ASK_FOR_ROW_SPACE_OR_TOT)  # con opzione "TOTAL"
            return False
                ...
                if self.pathMethod == QadARRAYCommandClassPathMethodTypeEnum.DIVIDE:
                    ...
                    if type(value) == int or type(value) == long:  # se é stato inserito il numero di elementi
                        if value * self.pathRows > maxArray:
                            ...
                            self.showErr(errMsg.format(
                                str(int(self.pathLinearObjectList.length() / self.distanceBetweenCols) + 1)))
                        else:
                            self.pathItemsNumber = value
                            self.distanceFromStartPt = 0.0
                            self.waitForMainOptions()
                    elif type(value) == unicode:
                        if value == QadMsg.translate("Command_ARRAY",
                                                     "Fill entire path") or value == "Fill entire path":
