...
from qad.qad_stretch_maptool import *
...
class QadSTRETCHCommandClass(QadCommandClass):
    ...
    def waitForObjectSel(self):
        ...
        prompt = prompt + QadMsg.translate("Command_STRETCH",
                                           " to stretch crossed by a selection window or [{0}]: ").format(keyWords)
    ...
    def run(self, msgMapTool=False, msg=None):
            ...
            if type(value) == QgsPoint:
                ...
                if self.AddOnSelection == False:
                    ...
                    if shiftKey:  # se la selezione é avvenuta con shift premuto
                        self.addEntitySetGeom(selSet,
                                              QgsGeometry.fromRect(QgsRectangle(self.points[0], self.points[1])))
                    else:
                        self.setEntitySetGeom(selSet,
                                              QgsGeometry.fromRect(QgsRectangle(self.points[0], self.points[1])))
                # si appresta ad attendere la selezione degli oggetti da stirare
                self.waitForObjectSel()
            return False
        ...
        # RISPOSTA ALLA RICHIESTA PUNTO BASE (da step = 1)
            ...
            if value is None or type(value) == unicode:
                ...
                # si appresta ad attendere un punto
                msg = QadMsg.translate("Command_STRETCH",
                ...
                # si appresta ad attendere un punto o enter o una parola chiave
                # msg, inputType, default, keyWords, nessun controllo
                self.waitFor(QadMsg.translate("Command_STRETCH",
                                              "Specify second point or [Array] <use first point as displacement from origin point 0,0>: "), \
                             QadInputTypeEnum.POINT2D, \
                             None, \
                             "", QadInputModeEnum.NONE)
                self.step = 6
