...
class QadGripMarker(QgsMapCanvasItem):
    ...
    def __init__(self, mapCanvas):
        ...
        self.borderColor = QadVariables.get(
            QadMsg.translate("Environment variables", "GRIPCONTOUR"))  # color of the border
    ...
    def paint(self, painter, option, widget):
        ...
        if self.__iconType == QadGripIconTypeEnum.NONE:
            ...
            # un quadrato
            painter.fillRect(-self.iconSize, -self.iconSize, self.iconSize * 2, self.iconSize * 2,
            ...
            # un rettangolo
            painter.fillRect(-self.iconSize, -self.iconSize / 2, self.iconSize * 2, self.iconSize,
                             QBrush(QColor(self.fillColor)));
...
class QadEntityGripPoints(QgsMapCanvasItem):
    ...
    def initGripPoints(self, grips=2):
        ...
        if wkbType == QGis.WKBPoint:
            ...
            coordTransform = QgsCoordinateTransform(self.entity.layer.crs(),
    ...
    def getGripPointsFromQadLinearObjectList(self, linearObjectList, atGeom=0, atSubGeom=0, grips=2):
        ...
        while nVertex < linearObjectList.qty():
            ...
            if isClosed == False and nVertex == 0:
                gp = QadEntityGripPoint(self.mapCanvas, startPt, QadGripPointTypeEnum.END_VERTEX, atGeom, atSubGeom,
                                        nVertex)
            else:
                gp = QadEntityGripPoint(self.mapCanvas, startPt, QadGripPointTypeEnum.VERTEX, atGeom, atSubGeom,
                                        nVertex)
            result.append(gp)
            if grips == 2:
                ...
                if linearObject.isSegment():  # linea
                    gp = QadEntityGripPoint(self.mapCanvas, middlePt, QadGripPointTypeEnum.LINE_MID_POINT, atGeom,
                                            atSubGeom, nVertex, rot)
                else:  # arco
                    gp = QadEntityGripPoint(self.mapCanvas, middlePt, QadGripPointTypeEnum.ARC_MID_POINT, atGeom,
                                            atSubGeom, nVertex, rot)
                result.append(gp)
            nVertex = nVertex + 1
